        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 min-h-[300px]">
            ...
            projects.map((project, idx) => (
              <AnimatedSection key={project.id} delay={idx * 0.15} className="bg-gray-800/40 backdrop-blur-sm border border-gray-700 rounded-xl overflow-hidden hover:border-indigo-500/50 hover:shadow-[0_8px_30px_rgba(79,70,229,0.15)] transition-all duration-300 group flex flex-col h-full">
                
                <Link to={`/projects/${project.slug}`} className="flex flex-col flex-grow h-full no-underline group/link">
                  ...
                  <div className="w-full h-48 bg-gray-900 border-b border-gray-700 flex items-center justify-center relative overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/20 to-purple-600/20 group-hover/link:scale-110 transition-transform duration-500"></div>
                    {project.image ? (
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    <h3 className="text-2xl font-bold text-white group-hover/link:text-indigo-300 transition-colors duration-300 mb-3">{project.title}</h3>
                    ...
                    <div className="border-t border-gray-700/50 pt-4 flex items-center justify-between">
                      <div className="flex gap-3 text-gray-400">
                        {project.githubUrl && project.githubUrl !== '#' && (
                          <a href={project.githubUrl} target="_blank" rel="noreferrer" className="hover:text-white transition-colors" onClick={(e) => e.stopPropagation()}>
                            <GitBranch className="w-5 h-5" />
                          </a>
                        )}
                        {project.liveUrl && project.liveUrl !== '#' && (
                          <a href={project.liveUrl} target="_blank" rel="noreferrer" className="hover:text-white transition-colors" onClick={(e) => e.stopPropagation()}>
                            <ExternalLink className="w-5 h-5" />
                          </a>
                        )}
                      </div>
                      <span className="text-indigo-400 group-hover/link:text-indigo-200 text-sm font-medium flex items-center transition-colors">
                        Détails <ArrowRight className="w-4 h-4 ml-1 transform group-hover/link:translate-x-1 transition-transform" />
                      </span>
                    </div>
                  </div>
                </Link>
